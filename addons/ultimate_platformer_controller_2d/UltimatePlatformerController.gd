...
@export var README: String = "Usage tips. 1. Hover over each toggle and variable to read what it does and to make sure nothing bugs. 2. Animations are very primitive. To make full use of your custom art, you may want to slightly change the code for the animations"
#INFO READEME 
#Usage tips. 
...
func _physics_process(delta):
	...
	upHold = Input.is_action_pressed("foward")
	downHold = Input.is_action_pressed("backward")
	...
	dashTap = Input.is_action_just_pressed("sprint")
	rollTap = Input.is_action_just_pressed("roll")
	downTap = Input.is_action_just_pressed("backward")
	...
	if eightWayDash and dashTap and dashCount > 0 and !rolling:
		var input_direction = Input.get_vector("left", "right", "foward", "backward")
