class_name FirstPersonController3D extends CharacterBody3D
...
@export_range(0.1, 9.0, 0.1, "or_greater") var joystick_sens: float = 3
...
func _handle_joypad_camera_rotation(delta: float, sens_mod: float = 1.0) -> void :
	...
	if joypad_dir.length() > 0:
		look_dir += joypad_dir * joystick_sens * delta
...
func set_state(new : String) -> void:
	state = new
